...
import {
    ...
    Avatar, Chip, Tooltip, Select, MenuItem, FormControl,
    InputLabel,
} from '@mui/material';
import {
    ChevronLeft, ChevronRight, AddPhotoAlternate,
    ExpandMore, ExpandLess, FreeBreakfast, LunchDining,
...
const MOCK_DATA = {
    '2026-03-01': {
        ...
        breakfast: {
            foods: [{ name: '토스트', calories: 200 }, { name: '아보카도', calories: 120 }, { name: '아메리카노', calories: 10 }],
            nutrients: { carbs: 30, protein: 7, fat: 18, sugar: 4 },
            memo: '',
        },
        lunch: {
            foods: [{ name: '비빔밥', calories: 540 }, { name: '된장국', calories: 45 }],
            nutrients: { carbs: 75, protein: 20, fat: 14, sugar: 8 },
            memo: '학식 점심',
        },
        dinner: {
            foods: [{ name: '닭볶음탕', calories: 380 }, { name: '현미밥', calories: 210 }],
    ...
    '2026-03-02': {
        ...
        breakfast: {
            foods: [{ name: '계란 2개', calories: 140 }, { name: '식빵', calories: 130 }, { name: '우유', calories: 120 }],
            nutrients: { carbs: 28, protein: 18, fat: 14, sugar: 12 },
            memo: '',
        },
        lunch: {
            foods: [{ name: '김밥', calories: 380 }, { name: '라면', calories: 500 }],
            nutrients: { carbs: 90, protein: 18, fat: 24, sugar: 8 },
            memo: '편의점 점심 😅',
        },
        dinner: {
            foods: [{ name: '치킨', calories: 560 }, { name: '맥주', calories: 140 }],
...
// ─── 상수 ────────────────────────────────────────────────────────────────────
const MEALS = [
    { key: 'breakfast', label: '아침', Icon: FreeBreakfast, color: '#FF8A65', bg: '#FFF3E0', darkColor: '#E64A19' },
    { key: 'lunch', label: '점심', Icon: LunchDining, color: '#66BB6A', bg: '#E8F5E9', darkColor: '#2E7D32' },
    { key: 'dinner', label: '저녁', Icon: DinnerDining, color: '#7986CB', bg: '#E8EAF6', darkColor: '#303F9F' },
    { key: 'snack', label: '간식', Icon: Icecream, color: '#F06292', bg: '#FCE4EC', darkColor: '#C2185B' },
];
    ...
    return (
        ...
        >
            ...
            >
                ...
                <Box sx={{ flexGrow: 1 }}>
                    <Typography fontWeight={700} fontSize="0.95rem">{meal.label}</Typography>
                    {data?.foods?.length > 0 && (
                ...
                {totalCal > 0 ? (
                    <Chip
                        icon={<LocalFireDepartment sx={{ fontSize: '14px !important', color: `${darkColor} !important` }} />}
                        label={`${totalCal} kcal`}
                        size="small"
                        sx={{ bgcolor: bg, color: darkColor, fontWeight: 700, fontSize: '0.75rem' }}
                    />
                ) : (
                    <Chip label="미기록" size="small" variant="outlined" sx={{ color: 'text.disabled', borderColor: '#e2e8f0' }} />
                )}
            ...
            <Collapse in={open}>
                ...
                <Box sx={{ p: 2.5 }}>
                    ...
                    {data?.foods?.length > 0 ? (
                        ...
                        <Box sx={{ bgcolor: '#f8fafc', borderRadius: 2, p: 2, textAlign: 'center', mb: 2 }}>
                            <Typography variant="body2" color="text.disabled">아직 기록된 음식이 없습니다.</Typography>
                        </Box>
                    )}
                        ...
                        <>
                            ...
                            <Stack spacing={1.2} mb={2}>
                                {NUTRIENT_CONFIG.map((n) => (
                                    <NutrientBar key={n.key} label={n.label} value={data.nutrients[n.key]} daily={n.daily} color={n.color} />
                                ))}
                            </Stack>
                        </>
                    )}
...
function AddRecordCard({ onSave, userId }) {
    ...
    return (
        ...
        >
            ...
            >
                <Avatar sx={{ width: 36, height: 36, bgcolor: '#fff3ed', color: '#FF8243' }}>
                    <Add sx={{ fontSize: 20 }} />
                </Avatar>
                <Typography fontWeight={600} color="#FF8243">기록 추가</Typography>
                <Box sx={{ flexGrow: 1 }} />
                    ...
                    <Stack spacing={2.5}>
                        ...
                        <Box>
                            ...
                            <Button
                                ...
                                sx={{
                                    mt: 1, color: '#FF8243', fontSize: '0.8rem',
                        ...
                        {totalCalories > 0 && (
                            <Box
                                sx={{
                                    ...
                                    bgcolor: '#fff3ed', borderRadius: 2, px: 2, py: 1.2,
                        ...
                        <Box sx={{ display: 'flex', gap: 1, justifyContent: 'flex-end' }}>
                            <Button
                                ...
                                sx={{ borderColor: '#e2e8f0', color: 'text.secondary', borderRadius: 2 }}
                            >
                                취소
                            </Button>
                            <Button
...
// ─── 달력 ────────────────────────────────────────────────────────────────────
function CustomCalendar({ selectedDate, onDateSelect, currentMonth, onMonthChange }) {
            ...
            <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', mb: 0.5 }}>
                {weekLabels.map((d, i) => (
                    <Typography
                        ...
                        align="center"
                        fontWeight={600}
                        sx={{ color: i === 0 ? '#EF5350' : i === 6 ? '#5C6BC0' : 'text.secondary' }}
                    >
                        {d}
                    </Typography>
                ))}
            ...
            <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 0.3 }}>
                {cells.map((day, idx) => {
                    ...
                    return (
                        <Tooltip
                            key={idx}
                            title={hasData ? `${MOCK_DATA[dateStr].summary.calories} kcal` : ''}
                            arrow
                            placement="top"
                        >
                            <Box
                                ...
                                sx={{
                                    ...
                                    bgcolor: isSelected
                                        ...
                                        : isToday
                                            ? '#fff3ed'
                                            : 'transparent',
                                    border: isToday && !isSelected ? '2px solid #FF8243' : '2px solid transparent',
                            ...
                            >
                                {isToday && isSelected && (
                                    <CheckCircle sx={{ position: 'absolute', top: 2, right: 2, fontSize: 10, color: '#fff' }} />
                                )}
                                <Typography
                                    variant="caption"
...
function NutritionSummaryPanel({ date, data }) {
    ...
    return (
        <Box sx={{ mt: 2.5, pt: 2.5, borderTop: '1px dashed #e2e8f0' }}>
            ...
            >
                {[
                    { label: '칼로리', value: `${calories} kcal`, color: '#FF8243', bg: '#fff3ed' },
                    { label: '탄수화물', value: `${carbs}g`, color: '#FFA726', bg: '#fff8f0' },
                    { label: '단백질', value: `${protein}g`, color: '#66BB6A', bg: '#f0faf0' },
                    { label: '지방', value: `${fat}g`, color: '#EF5350', bg: '#fff0f0' },
                    { label: '당류', value: `${sugar}g`, color: '#AB47BC', bg: '#faf0ff' },
                ].map((item) => (
                    <Box
                        key={item.label}
                        sx={{ bgcolor: item.bg, borderRadius: 2, p: 1.2, textAlign: 'center' }}
                    >
...
export default function DailyLogPage() {
    ...
    return (
        <Box sx={{ minHeight: '100vh', bgcolor: '#f5f7fa' }}>
            ...
            >
                <IconButton component={RouterLink} to="/login" size="small">
                ...
                {isToday && totalCalories > 0 && (
                    <Chip
                        icon={<LocalFireDepartment sx={{ fontSize: '16px !important', color: '#FF8243 !important' }} />}
                    ...
                    <Box sx={{ mb: 2.5, display: 'flex', alignItems: 'center', gap: 1.5 }}>
                        <Box>
                            <Typography component="div" fontWeight={800} fontSize="1.2rem">
                                ...
                                {isToday && (
                                    <Chip label="오늘" size="small" sx={{ ml: 1, bgcolor: '#FF8243', color: '#fff', fontWeight: 700, fontSize: '0.7rem' }} />
                                )}
                            </Typography>
                            {totalCalories > 0 && (
                    ...
                    <Stack spacing={0}>
                        {MEALS.map((meal, index) => (
                            <Box key={meal.key} sx={{ display: 'flex', gap: 2 }}>
                                {/* 타임라인 인디케이터 */}
                                <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: 20, flexShrink: 0, mt: 2 }}>
                                    <Box
                                        sx={{
                                            width: 12, height: 12, borderRadius: '50%',
                                            bgcolor: dayData?.[meal.key]?.foods?.length ? meal.color : '#e2e8f0',
                                    ...
                                    {index < MEALS.length - 1 && (
                                        <Box sx={{ width: 2, flexGrow: 1, bgcolor: '#e8ecf0', my: 0.5, minHeight: 24 }} />
                                    )}
                                </Box>
